...
`d0549fe Surface rename impact in the pre-patch plan` on `main`.
...
- **Item 3 — Pre-patch blast radius and edit preview.** The plan
  ...
  `preview_patch` model tool. `/plan` CLI shortcut. Rename impact
  analysis (covered in item 8) feeds `expected_scope.rename_impact`
  and a `rename_impact` warning. **Open**: a "preview" review
  action that shows the next staged patch before apply, and
  signature-change impact (parameter list / return type rewrites).
...
- Most recent slice: **rename impact in the pre-patch plan**
  (commit `d0549fe`). When the user request mentions `\brename\b`
  and the named symbol is defined in the workspace, the plan walks
  the import graph through `listSymbolImpact` (defined in
  `src/code/code-index.js`) and surfaces a `rename_impact` warning
  — blocking when the symbol has cross-file callers, informational
  when only the definition is affected. Affected files land in
  `expected_scope.candidate_files` so they participate in the same
  danger-zone crosses as keyword candidates. 4 new tests in
  `test/pre-patch-plan.test.js` lock the behaviour in.
- Slice before that: **cross-file symbol binding** (commit
  `31a4878`) — shared `src/code/import-resolver.js`, plus
  `findSymbolCallers` / `findSymbolDependencies` exposed as
  `tools.*` and as the `symbol_callers` / `symbol_dependencies`
  model tools.
- `.agent/` is runtime state and is gitignored. Project memory and
  task artifacts can be regenerated by the harness.

### Next slice — repair-loop integration with the import graph

This is the recommended next item-8 slice. The repair loop already
hands `model.repair` a compact summary
(`summarizeFailureForRepair` in `src/checks/failure-summary.js`)
that includes `likely_relevant_files` (re-ranked by
`src/checks/relevant-files.js`). The missing piece: pass the
*resolved import graph* of each failing test file so the model
sees which workspace files the test actually pulls in, not just
files that share names.

Concrete starting point:

1. In `src/checks/failure-summary.js`, extend
   `summarizeFailureForRepair(parsed, { codeIndex } = {})` to call
   `findSymbolDependencies({ codeIndex, file })` for every entry
   in `parsed.failed_files` and attach the resolved internal
   dependencies as `import_graph: { <test-file>: [<resolved>...] }`
   on the summary. Skip files not in the index, and skip the
   external (bare-specifier) entries.
2. In `src/verify/repair-loop.js`, thread the runtime's cached
   `codeIndex` (already available — see how `runCheckCommand`
   reads it) through to `summarizeFailureForRepair`.
3. Add tests in `test/failure-summary.test.js` that build a tiny
   index by hand (Set-style mock like the rename-impact tests use)
   and assert that a failed test file gets its resolved imports
   attached. Add a repair-loop integration test in
   `test/repair-loop.test.js` verifying `model.repair` receives
   the new field.
4. Run `npm test` (expected count rises by ~3) and
   `npm run test:e2e`. Update README + `phase 3.txt` item 8 to
   tick the repair-loop sub-bullet, bump the test count, and
   record the new commit hash. Commit and push.

### Next recommended work after that
...
1. Item 7 leftover — implement `/resume <task-id>`. Extract the
   ...
   on disk. The phase controller already supports this (every
   phase artifact persists; `validateCanBegin` accepts skipped or
   completed predecessors).
2. Item 8 leftovers after the repair-loop integration:
   signature-change impact (rename is covered; argument-list /
   ...
   resolution against the package layout.
3. Stragglers (in priority order):
   ...
   - Streaming-aware `ui.assistant` block + streaming the repair
     loop (item 4).
   - "Preview" review action that shows the next staged patch
     before apply (item 3).
   - pytest JUnit wiring (needs tmp-file capture since pytest
     doesn't write JUnit to stdout) and ESLint structured wiring
     (item 2).
4. Items 9 (real LSP integration, TypeScript first) and 10
   (GitHub/CI integration) are sized as their own future rounds.
