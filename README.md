...
- **Item 3 — Pre-patch blast radius and edit preview.** The plan
  ...
  and a `rename_impact` warning. Signature-change analysis now feeds
  `expected_scope.signature_impact` and a `signature_impact` warning.
  **Open**: a "preview" review action that shows the next staged
  patch before apply.
...
- Most recent slice: **signature-change impact**. `buildPrePatchPlan`
  ...
  `expected_scope.candidate_files`, stores the structured impact under
...
- Slice before that: **React component map tool**. `component_map()`
  ...
  runtime integration.
- Earlier slice: **dependency graph tool**. `dependency_graph(path?)`
