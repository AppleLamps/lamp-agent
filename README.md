...
Most recent change is the `component_map()` tool. It detects common
React function/class/arrow component declarations and links JSX tags
inside component bodies to local/imported components when the import
graph can resolve the target file.

Latest verified baseline: `npm test` passes with 216 tests
...
- **Item 8 — Dependency-aware edits and cross-file binding.** Shared
  ...
  `component_map` is now available as a runtime and model tool,
  returning best-effort React component declarations plus JSX render
  edges between local and imported components. **Open**: signature-
  change impact (renames are covered; argument-list changes are not),
...
- Most recent slice: **React component map tool**. `component_map()`
  ...
  declarations and JSX render edges between local/imported
  components. Tests cover component discovery, render edges, and
  runtime integration.
- Slice before that: **dependency graph tool**. `dependency_graph(path?)`
  returns the workspace import graph, or a focused dependency
  subgraph plus direct dependents when a path is supplied.
- Earlier slice: **resumable task command**. `/resume <task-id>`
...
- Earlier item-8 slice: **repair-loop import graph integration**.
  ...
  exposes the runtime's cached code index to that summarizer before
  calling `model.repair`. New tests in `test/failure-summary.test.js`
  and `test/repair-loop.test.js` lock the payload shape.
- Earlier item-8 slice: **rename impact in the pre-patch plan**
  (commit `d0549fe`). Rename intent walks the import graph through
  `listSymbolImpact` and surfaces a `rename_impact` warning before
  patching.
- Earlier item-8 slice: **cross-file symbol binding** (commit
...
### Next slice — remaining item 8 impact work
...
1. Item 8 leftovers:
   ...
   diff), tsconfig-paths alias resolution (extend
